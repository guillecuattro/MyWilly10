...
export default function MyWilly() {
  ...
  const callGemini = async (prompt, useSearch=true, maxTokens=1500) => {
    ...
    if (!key) throw new Error("Configura tu API key de Gemini en Ajustes.");
...
Incluye entre 4 y 6 noticias. Busca eventos reales de hoy: geopolítica, bancos centrales, inflación, divisas, sectores clave. Relaciona cada noticia con los fondos específicos del usuario.`;
      ...
      setNewsError(hasKey ? "Error al cargar noticias. Comprueba tu conexión." : "API key de Gemini no configurada. Pulsa el icono ⚙ en la cabecera para añadirla.");
    }
    ...
    <>
      ...
      <div style={{height:"100svh",background:C.bg,fontFamily:"'Plus Jakarta Sans',sans-serif",color:C.text,maxWidth:ip?820:430,margin:"0 auto",display:"flex",flexDirection:"column",paddingTop:"env(safe-area-inset-top, 0px)",overflow:"hidden",position:"relative"}}>
        ...
        <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:ip?820:430,background:C.surface,borderTop:`1px solid ${C.border}`,zIndex:100,paddingBottom:`env(safe-area-inset-bottom, 0px)`}}>
          {/* Row 1 — centred vertically in its half */}
          <div style={{display:"flex",justifyContent:"space-around",alignItems:"center",height:44}}>
            {[{id:"home",label:"Resumen",icon:<IHome/>},{id:"funds",label:"Mis fondos",icon:<IFunds/>}].map(n=>(
              <button key={n.id} onClick={()=>setTab(n.id)} style={{background:"none",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,color:tab===n.id?C.green:C.muted,fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:9,fontWeight:600,flex:1,height:"100%",transition:"color 0.16s"}}>
                {n.icon}<span>{n.label}</span>
              </button>
            ))}
          </div>
          {/* Divider */}
          <div style={{height:"1px",background:C.border,opacity:0.6,margin:"0 5%"}}/>
          {/* Row 2 — centred vertically in its half */}
          <div style={{display:"flex",justifyContent:"space-around",alignItems:"center",height:44}}>
            {[{id:"fiscal",label:"Fiscal",icon:<ITax/>},{id:"widget",label:"Widget",icon:<IWidget/>},{id:"news",label:"Noticias",icon:<INews/>}].map(n=>(
              <button key={n.id} onClick={()=>setTab(n.id)} style={{background:"none",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:2,color:tab===n.id?C.green:C.muted,fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:9,fontWeight:600,flex:1,height:"100%",transition:"color 0.16s"}}>
                {n.icon}<span>{n.label}</span>
              </button>
            ))}
